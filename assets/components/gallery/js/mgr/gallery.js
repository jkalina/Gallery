...
GAL.window.CompressUpload = function(config) {
    config = config || {};
    this.ident = config.ident || 'gupit'+Ext.id();
    Ext.applyIf(config,{
        title: _('gallery.compress_and_upload')
        ,id: this.ident
        // ,height: 150
        // ,width: '55%'
        ,width: 600
    ...
    GAL.window.CompressUpload.superclass.constructor.call(this,config);
    this.on('activate',function() {
        if (typeof Tiny != 'undefined') { MODx.loadRTE(this.ident + '-description'); }
    });
};
Ext.extend(GAL.window.CompressUpload,MODx.Window);
Ext.reg('gal-window-compress-upload',GAL.window.CompressUpload);